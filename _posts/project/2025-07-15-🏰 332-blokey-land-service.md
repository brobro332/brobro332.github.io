...
###  **개발 기간**
- `v1.0.0` (`2025-06-06 ~ 2025-06-15`): 백엔드드 설계 및 초기 기능 구현
...
#### ✅ 프론트엔드 상태 관리
- 가령 `A → B → C` 방향으로 컴포넌트를 호출하고 `A`의 상태를 `C`에서 조작해야 할 경우 `B`에서 그 흐름이 끊기면 컴파일을 통해 문제를 디버깅 할 수 없어 문제를 해결하는데 오랜 시간이 걸렸다.
...
- 컨테이너를 자식 클래스 개수 만큼 재생성하면, 리소스도 커지고 실제로 컨테이너 생성 시간 때문에 `DB` 연결 문제로 테스트도 실패했다.
...
- 이를 위해 프로젝트 및 사용자 도메인에 스킬(`skill`), 포지션(`discipline`) 필드를 추가하고, `ElasticSearch`를 도입하여 프로젝트와 멤버 간 추천 및 매칭 기능을 개발하고자 한다.
...
#### ✅ 디자인
- 초기에는 사용자에게 친근한 인상을 주기 위해 `Blokey-Land`라는 네이밍과 게이미피케이션 디자인을 적용했으나, 사이트에 처음 방문하는 사용자가 서비스 목적을 직관적으로 이해하기 어려울 것 같다는 생각이 들었다.
