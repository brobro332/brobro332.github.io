...
- 홈 화면에 접속하여 버튼을 누르면 사용자는 퀴즈를 생성하는 출제자가 된다.
...
##### 사용자 도메인의 생략은 오히려 프로세스를 복잡하게 하였다.
...
- 세션 내 접속 및 이탈은 서버 메모리 상에서 관리하고자 했지만,식별 키로 사용된 `Socket ID`는 고유성이 부족하여, 브라우저 새로고침이나 강제 종료와 같은 케이스에서는 이탈을 정확히 감지할 수 없었다.
